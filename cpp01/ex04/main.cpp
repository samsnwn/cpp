...
std::string replaceString(const std::string &line, const std::string &s1,
                          const std::string &s2) {
  if (s1.empty())
    return line;
...
bool sed(const std::string &filename, const std::string &s1,
         const std::string &s2) {
  if (filename.empty()) {
  ...
  if (!in.eof() && in.fail()) {
    std::cerr << "Error: failed to read from file '" << filename << "'"
              << std::endl;
    in.close();
    out.close();
    return false;
